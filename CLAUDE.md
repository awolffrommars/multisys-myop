...
- **Deploy:** Up to date as of 2026-09-10. Recent chain: `6194020` (loose photo matching + PDF ZIPs + browser/memory/ownership fixes) → `baaf1aa` (dev:noauth) → `9a2ddb5` (doc sweep) → `e1a13b3` (Start Over restores template; required-field doc fixes) → `3989afb` (signature required in ID manual entry) → this commit (photo required in ID manual entry). Includes the 2026-07-14 hardening batch, see `HARDENING-2026-07-14.md`. Use `./deploy.sh "message"` to commit + ship both remotes. Revert hatch: `git reset --hard backup-pre-hardening`.
  **Do NOT push until the user asks** — changes batch up locally. When they do ask, sweep every affected file FIRST (CLAUDE.md, README.md, SETUP.md, HANDOFF.md, package.json, and the memory files) and then commit once; edits made after `./deploy.sh` runs leave a trailing dirty tree, which happened twice on 2026-09-02.
  **`deploy.sh` pushes GitHub first, HuggingFace second, with no rollback between them** — a mid-script failure (e.g. expired git credentials, as happened 2026-09-08) leaves the Space behind GitHub. Always confirm both remotes after a deploy that errored. If GitHub auth fails, `gh auth setup-git` points git at the existing `gh` login.
...
**In-memory job store:** `jobs` Map keyed by UUID. Each job holds `{ employees, photoMap, signatureMap, posters[], photos[], signatures[], status, templateKey, owner }`. `photos[]` and `signatures[]` store `{buffer, format}` **references** to bytes already held by `photoMap`/`signatureMap` (1:1 index with `posters[]`) so `/regenerate` can re-use the originals without re-uploading. **Do NOT store `{base64, format}` there** — a base64 string is a ~1.33x-sized second copy of every upload, and a 500-employee ID batch held ~6.5 GB of pure duplication. `toRenderData(ref)` converts to the renderer's `{base64, format}` shape at render time only (and passes through an already-base64 object defensively). Puppeteer browser singleton lives for one batch (`closeBrowser()` in the `finally` block).
...
- **Image decode guard (do NOT remove):** after `document.fonts.ready`, `renderPoster` awaits `img.decode()` for every `document.images` entry, then two `requestAnimationFrame` ticks before `page.screenshot()`. `setContent(..., {waitUntil:'load'})` only guarantees bytes arrived, not that the bitmap is decoded/composited — a 10 MB 2500x4300 portrait on the 1920x3050 ID canvas can otherwise capture half-painted. Added 2026-08-20 as hardening; the failure was NOT reproduced locally (8/8 renders complete both with and without it), so it is defensive, not a confirmed fix.
- **Browser lifecycle (do NOT revert to a resolved-browser singleton):** `getBrowser()` memoizes the launch **promise** (`browserPromise`), not the resolved browser. The old `if (!browser) browser = await puppeteer.launch()` let both renders of a `CONCURRENCY=2` `Promise.all` pass the null check before either awaited — two Chromiums launched, the second assignment orphaned the first, and it survived as a ~80 MB process reparented to init. Confirmed live (orphans up to 3 days old on the dev machine). A `disconnected` listener clears the memo so a crashed browser isn't cached as a dead handle, and a failed launch clears it so the next call retries. `closeBrowser()` is still a no-op while `activeRenders > 0` (two concurrent batches share the browser; unconditional close killed the other batch with "Target closed") and now clears the memo *before* awaiting the close. Every render's page is closed in a `finally` (error paths leaked pages).
...
- **Multisys ID:** Employee photo required. Signature optional at render time (required in manual entry since 2026-09-08, still permissive for batch) — uploaded separately (batch zone or manual file input), matched by employee name same as photos. `{{SIGNATURE_BASE64}}` token; when absent, `#sigOverlay` is hidden via injected CSS (`display:none`). Tokens cover Employee Information (employeeNumber, address, phoneNumber, philhealth, sss, tin, hdmf) and Emergency Contact (contactName, contactAddress, contactNumber). Viewport: 1920×1080.
...
**Photo matching:** `services/matcher.js` exports `normalizeNameKey(str)` — strips diacritics via NFD decomposition (`normalize('NFD').replace(/[̀-ͯ]/g,'')`), lowercases, strips commas/separators, sorts tokens. Diacritic stripping means "Escaño" and "Escano" match, "ñ"→"n", "é"→"e", etc.

**Loose (subset) matching — added 2026-09-02.** Exact key equality alone rejected real filenames (`Nama-Sy, Ma. Benjieleen Tenido_3.png`, partial names like `Garlan, Peter.png` for "Peter Psalm Garlan"). The rule now:
- `nameTokens()` drops export junk (`copy`, `final`, `v2`, `img`, …), bare numbers (`_3`, `(2)`) and single letters (middle initials).
- `scoreMatch(employee, file)` matches only when **every meaningful token of the shorter name appears in the longer one** AND **2+ tokens are shared** — unless both names are a single token. So `Garlan, Peter` ✓ `Peter Psalm Garlan`; `John Cruz` ✗ `John Santos` (1 shared); `mila-kath` ✗ `Kathleen Mila` (no prefix/truncation matching — deliberate); `dela Cruz, Maria` ✗ `Jose dela Cruz` (2 shared but neither is a subset — this is what protects shared Filipino surnames).
- **`resolveMatches(names, map)` resolves the whole batch, not one employee at a time:** exact matches claim their file first, each remaining employee takes its uniquely best *unclaimed* file, then any file wanted by two employees is withdrawn from both. Per-employee matching cannot see those collisions and would hand one photo to two people — a wrong face on an ID card.
- Ambiguity is never guessed: those employees report as unmatched and `/prepare` returns `ambiguousMatches[]`, shown in the warning banner next to `duplicateFiles`.
- `/prepare` stores the resolution on the job (`photoMatches` / `signatureMatches`) and **`/generate` renders from it** so the output cannot disagree with the preview. `/regenerate` keeps both the map and the resolution in step when a photo/signature is uploaded.
- **The client mirrors `JUNK_TOKENS` / `nameTokens` / `looseNameMatch` in `index.html`** for the photo-list badge — keep the two in sync or the badge lies about what the server will do. A parity test over 27 name/file pairs confirmed identical verdicts. `buildPhotoMap` attaches a `duplicates` array to the returned Map (two files normalizing to the same key — last wins); `/prepare` returns them as `duplicateFiles` and the client shows a warning banner.
...
**ZIP download for paired templates:** `multisys-id` and `calling-card` (anything with a `PDF_CONFIG` entry) ZIP as **one 2-page PDF per employee**, not PNG sides — matching their single-poster "Save PDF" button. `/download` pairs each `side !== 'back'` poster with the following `back` and calls `renderPdfBatch()`. `buildZip()` takes an `ext` per entry (defaults to `png`) — the other three templates are unchanged. Use `renderPdfBatch` (one browser for the whole set), NOT `renderPdf` in a loop: `renderPdf` launches *and closes* a browser per call, so a 50-ID ZIP would pay 50 launches.
...
**Years pluralisation:** the template renders `{{ANNIVERSARY_YEARS}} {{YEARS_WORD}}` — it used to hardcode ` YEARS`, which produced "1 YEARS". `poster.js` sets `{{YEARS_WORD}}` to `YEAR` when the value parses to exactly 1, `YEARS` otherwise (non-numeric and blank keep the plural, matching the old behaviour). Do NOT put the word back in the template.

Fields: `{{TEMPLATE_BASE64}}`, `{{PHOTO_BASE64}}`, `{{FIRST_NAME}}`, `{{LAST_NAME}}`, `{{POSITION}}`, `{{DIVISION}}`, `{{DEPARTMENT}}`, `{{ANNIVERSARY_YEARS}}`, `{{YEARS_WORD}}`.
...
**Fonts are embedded (do NOT replace with a remote `<link>`):** the file carries the same 5 base64 woff2 Montserrat subsets (variable, `font-weight: 400 800`) as poster-birthday / poster-multisys-id. It previously pulled Montserrat from `fonts.googleapis.com` **at render time** — the only template that did — so with no egress or a slow response the card silently fell back to Arial, and the AUTOFIT pass then measured the fallback and baked in wrong font sizes. File is ~228 KB as a result, same as the other templates.
...
- **Batch Upload**: CSV + bulk PNG photos. Client-side CSV type detection fires on file select. A trash-icon (`.zone-remove-btn`) appears on the CSV zone, photo zone, and signature zone (multisys-id only) to remove files — centered inline SVG, matches photo list item × style. Photos shown in a list with match status (red = no CSV match, purple = duplicate). The "Upload & Preview" button changes to `<span class="btn-spinner"></span> Preparing…` while `/prepare` is in flight. The inline "Upload Photo" button in the Step 2 preview table shows a spinner + "Uploading…" text during the same request. Both are reset on error. **`clearAllFields()` must explicitly set `photoRemoveBtn.style.display = 'none'`** — omitting this leaves the × visible when the zone is empty after a template switch or back-navigation. For multisys-id, `#signatureZone` `margin-top` is set dynamically in `renderPhotoList()` — 12px when photos are present, 0 when the photo list is empty — so spacing above and below the zone stays visually balanced.
  ...
  **Template-aware photo zone copy:** `photoZoneLabel()` / `photoZoneSub()` return "Drop portrait PNGs here…" + "Employee ID portraits · PNG only…" for `multisys-id`, and the generic photo wording otherwise. All four sites that write this copy (markup default, `clearAllFields()`, `updatePhotoZone()`, `onTemplateChange()`) go through the helpers — don't re-inline the strings.
...
- **Manual Entry**: Fields vary by template. New Employee uses a simple 3-column grid (Full Name | Position | Department). Birthday and Anniversary use a **6-column grid** with two rows:
  ...
  - **Multisys ID** shows basic fields (`#manualFormRow`) + extended `#manualIdSection` (Employee Number, Address, Phone, SSS, TIN, Pag-ibig Number, PhilHealth + Emergency Contact section + Signature upload zone `#manualSignatureZone`). IDs: `manualEmployeeNumber`, `manualAddress`, `manualPhoneNumber`, `manualSss`, `manualTin`, `manualHdmf`, `manualPhilhealth`, `manualContactName`, `manualContactAddress`, `manualContactNumber`, `manualSignatureInput`. **Required: every field, nothing optional** — Full Name, Position, Employee Number, Address, Phone Number, PhilHealth Number, SSS Number, TIN, HDMF, Contact Name, Contact Address, Contact Number, **Photo and Signature** (verified against `missingFields` in `commitManualEntry`, 2026-09-08). Both upload zones show a red `*`: the signature via its section header, the photo inline in the label. The signature label is the `SIG_LABEL` constant ("Click to upload signature" + "PNG preferred for transparent backgrounds", matching the photo zone). The photo label comes from `manualPhotoLabelHtml()` — `Photo * — click or drop` for multisys-id, `Photo (optional) — click or drop` for every other template, since that one zone is shared; it is assigned with `innerHTML` (constant strings only) so the asterisk renders, and all five write sites plus `onTemplateChange()` go through the helper. The edit modal keeps its own `_sigHint` "(optional)" because replacing an existing signature genuinely is.
  ...
  Required fields show a red `*`: Full Name + Position always, plus Birthday (birthday); Years **and Date Hired** (anniversary); Email + Contact Number (calling-card); and for multisys-id the full set listed under its bullet above. Birthday field has live month-spelling autocorrect on blur (hint shown inline to the right). Same autocorrect applies in the Edit Poster modal. All inputs have `autocomplete="off"`. Validation warnings (confirm banner): empty department, empty division (birthday/anniversary only), filename mismatch, **duplicate name**. Clicking Edit on a listed employee while another is mid-edit **auto-saves** the current form first.

**Known quirk — `_fieldMap` focus target:** the multisys-id branch pushes `'Contact Number'` for a missing *emergency* contact, which `_fieldMap` maps to `manualMobile` (the calling-card field, hidden for this template). The `_el.offsetParent` guard stops it focusing a hidden input, so the error banner is still correct but focus simply doesn't move. The `'Contact Number (Emergency)'` key in `_fieldMap` is dead — nothing pushes that string.

**Step 2 CTA label:** `generateBtnLabel()` → "Generate Calling Cards" (calling-card), "Generate IDs" (multisys-id), "Generate Posters" (everything else). Set in both `renderPreview()` and `onTemplateChange()`. The Step 3 success banner ("All posters generated!", "N posters ready") and the ZIP names deliberately keep "poster" wording — user's call, 2026-08-20.

**Step 2 — Employee Preview**: table with Division and Birthday/DateHired/Years columns shown only for relevant templates. Email/Mobile shown for calling-card; Employee # shown for multisys-id; Department hidden for calling-card/multisys-id; Photo column hidden for calling-card; Signature column shown only for multisys-id. Photo column uses `id="thPhoto"` + `.th-photo` class (width 90px); upload column uses `id="thUpload"` + `.th-upload` (width 120px) — do NOT use nth-child selectors for these. All column header elements have IDs (`thDepartment`, `thDivision`, `thBirthday`, `thYears`, `thDateHired`, `thEmail`, `thMobile`, `thEmployeeNumber`, `thPhoto`, `thSignature`, `thUpload`) — both `onTemplateChange()` and `renderPreview()` manage visibility. The `✓ Found` badge in the Photo Match column has an inline trash button (`.photo-remove-btn`) — clicking removes the matched photo from `photoFiles` (CSV mode, same subset-token filter as inline upload) or clears `manualEmp.photoFile` (manual mode), then re-runs `submitPrepare()` so the row flips to `✗ Missing` with the upload button. The subset-token filter NEVER removes a file whose key exactly matches a *different* employee ("Peter Garlan.png" is a subset of "Peter Psalm Garlan" but may be Peter Garlan's own photo) — it lives in one place, `dropEmployeeFiles(files, emp)`, used by all four inline photo/signature upload+remove paths.

  **Signature column (multisys-id) mirrors the Photo column:** `✗ Missing` gets an inline `↑ Upload` button, `✓ Found` gets a trash button. Inline signature uploads run through `removeSigBackground()` — the same strip the batch drop zone and manual entry apply — so an inline signature is processed identically to a batched one.

  **Cell-scoped selectors (do NOT revert to unscoped):** a row can now hold two file inputs and two trash buttons, so handlers bind via `.photo-cell` / `.sig-cell` / `.upload-cell` (e.g. `tr.querySelector('.upload-cell input[type="file"]')`). The old `tr.querySelector('input[type="file"]')` / `('.photo-remove-btn')` grabbed the first match in the row and would mis-wire the photo handler onto the signature control when both were present. `submitPrepare()` uses a monotonic `_prepareSeq` token — out-of-order responses from overlapping calls are discarded.

**Client state guards (do not remove):** `popstate` falls back to step 1 when `currentJobId` is null (Back after Start Over reached a stale gallery whose Edit POSTed `/regenerate/null/0`); Start Over and `clearAllFields()` (template switch) clear `currentJobId`/`currentEmployees` + gallery DOM. **Start Over then re-selects the template that was in use** — it captures `selectedTemplate` before the reset and dispatches a real `click()` on that option, so the restore runs the same path as a manual pick (active state, `aria-pressed`, card unlock, `onTemplateChange()`) instead of duplicating it; `selectOpt` sees `selectedTemplate === null` and therefore does NOT `clearAllFields()` a second time. When the restored template lives in the collapsed "More Templates" row (calling-card, multisys-id) that row is expanded too, or the selection would be highlighted out of sight. `showGallery(jobId, count, names)` builds cards from the server's rendered-names list (falls back to `photoFound` filter when absent — dev reload path). Lightbox uses `_posterVersions[index]` for cache-busting (set on regenerate) — NOT `Date.now()`, which re-downloaded every full PNG per prev/next. Manual-entry edit-load strips `MTC-`/`+63` prefixes and rebuilds extra mobile rows + signature state; the mid-edit auto-save normalizes identically to `commitEmployee` (prefixes, joined mobiles, `signatureFile`). **Photo/signature requirement history:** signatures were made optional everywhere 2026-07-13 (`/prepare` block and manual-entry requirement both removed), then **manual entry made required again 2026-09-08** for both the signature and the photo, at the user's request — an ID is not valid without either. Batch upload is deliberately still permissive: a missing photo or signature shows `✗ Missing` with an inline upload button, and generation skips photo-less employees / hides `#sigOverlay`. So manual entry blocks, batch warns — if that asymmetry ever needs closing, the `/prepare` side is where to add it. Note the "No photo uploaded — add anyway?" confirm in `#deptConfirm` is now unreachable for multisys-id (the required check returns first) but still applies to the other templates.
...
**Job ownership (`ownedJob`):** `/prepare` stamps `owner: req.user?.email` on the job. Every route that resolves a job from the URL — `/generate`, `/regenerate`, `/job/:id`, `/job/:id/reset`, `/preview`, `/download`, `/download-pdf` — goes through `ownedJob(req, jobId)` instead of `jobs.get()`. `requireAccess` only proves the caller is *an* approved user; without the owner check any approved user holding a jobId could read another user's job, and `/job/:jobId` returns the full employee array (addresses, SSS/TIN/PhilHealth, emergency contacts). Returns `null` for "not yours" as well as "not found" so the 404 doesn't confirm the id exists. Falls open when `AUTH_ENABLED` is false (local dev has no identity to check).
...
**KNOWN LIMITATION — session store is the default `MemoryStore`** (decision: leave as-is, 2026-08-28). Consequences: every restart or HuggingFace deploy signs all users out, and express-session's MemoryStore grows without bound (it never prunes expired sessions), so a long-lived Space slowly leaks. Not a correctness or security bug — the 8h `maxAge` and the signed cookie still hold — just re-login friction plus slow growth.
If it becomes worth fixing, the cheapest route is a small custom `express-session` Store on top of the libsql client already in `services/db.js` (no new dependency): a `sessions (sid TEXT PRIMARY KEY, data TEXT, expires INTEGER)` table plus `get`/`set`/`destroy`/`touch` methods, and an index on `expires` for periodic pruning. Avoid file-backed stores (`session-file-store`, `connect-sqlite3`) — HuggingFace Space disk is ephemeral, so they'd buy nothing over MemoryStore across deploys. Whoever does it must verify the Google sign-in flow by hand; it cannot be tested without live OAuth.
...
**Multer upload limits:** `/prepare` enforces `fileSize: 20 MB` and `files: 1001` (500 photos + 500 signatures + 1 CSV), plus a 300 MB total Content-Length cap (413) before multer buffers into memory. The old `501` silently rejected any Multisys ID batch that supplied a signature per employee. Multer limit breaches (`LIMIT_FILE_SIZE`, `LIMIT_FILE_COUNT`, `LIMIT_UNEXPECTED_FILE`, …) are translated to 413/400 with actionable text in the JSON error middleware — they used to fall through as a bare "Internal server error".

**`safeImageFormat(mimetype)`:** `/regenerate` derives the image format from the client-supplied upload mimetype, and it lands in `data:image/${format};base64,…` inside a `src=""` attribute in the render browser. An allowlist (`png|jpeg|jpg|webp|gif|avif`, default `png`) prevents a crafted mimetype from breaking out of the attribute. Batch uploads were never affected (`matcher.js` hardcodes `'png'`).

**Stale generation recovery:** a run whose server died mid-batch left `job.status === 'generating'`, and the `/generate` re-entry guard then closed every EventSource reconnect empty — the client retried every ~3s until the 2-hour TTL evicted the job. `/generate` now resets a job to `'ready'` if it has been `'generating'` for more than `STALE_GENERATE_MS` (30 min). Deliberately generous: a legitimately slow batch must never be restarted underneath itself. **Note:** there is intentionally NO abort-on-client-disconnect — `req.on('close')` also fires on the transient disconnects that EventSource is designed to reconnect through, so aborting there would kill healthy runs.

**`/download-pdf/:empIdx` pairing:** locates the Nth `side !== 'back'` poster by scanning, then takes the adjacent `back`. The old `empIdx * 2` assumed every employee produced exactly two posters — if a back template failed to load, one poster was pushed per employee and every index past the first returned the WRONG PERSON's card.
...
## Pre-Approved Commands (`***` announcement rule)

Lives in `~/.claude/CLAUDE.md` + `~/.claude/settings.json` (user-level, applies to all projects) — not
here, because the same read-only commands are used across every ClaudeWorks project. Safe read-only
commands (localhost `curl`, `qlmanage -t`, `sips -g`, `node --check`, `py_compile`, package-manager
queries, `unzip -l`, `mkdir -p`, read-only ClickUp/Drive/Playwright MCP) run without a prompt and are
announced inline prefixed with `***`. Non-localhost `curl`, bare interpreters, and anything mutating
still prompt.
