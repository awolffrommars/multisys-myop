...
- **Manual Entry**: Fields vary by template. New Employee uses a simple 3-column grid (Full Name | Position | Department). Birthday and Anniversary use a **6-column grid** with two rows:
  ...
  - **Multisys ID** shows basic fields (`#manualFormRow`) + extended `#manualIdSection` (Employee Number, Address, Phone, SSS, TIN, Pag-ibig Number, PhilHealth + Emergency Contact section + Signature upload zone `#manualSignatureZone`). IDs: `manualEmployeeNumber`, `manualAddress`, `manualPhoneNumber`, `manualSss`, `manualTin`, `manualHdmf`, `manualPhilhealth`, `manualContactName`, `manualContactAddress`, `manualContactNumber`, `manualSignatureInput`. **Required: every text field except the photo and signature** — Full Name, Position, Employee Number, Address, Phone Number, PhilHealth Number, SSS Number, TIN, HDMF, Contact Name, Contact Address, Contact Number (verified against `missingFields` in `commitManualEntry`, 2026-09-08). Photo zone visible, photo and signature both optional.
