...
- **Manual Entry**: Fields vary by template. New Employee uses a simple 3-column grid (Full Name | Position | Department). Birthday and Anniversary use a **6-column grid** with two rows:
  ...
  - **Multisys ID** shows basic fields (`#manualFormRow`) + extended `#manualIdSection` (Employee Number, Address, Phone, SSS, TIN, Pag-ibig Number, PhilHealth + Emergency Contact section + Signature upload zone `#manualSignatureZone`). IDs: `manualEmployeeNumber`, `manualAddress`, `manualPhoneNumber`, `manualSss`, `manualTin`, `manualHdmf`, `manualPhilhealth`, `manualContactName`, `manualContactAddress`, `manualContactNumber`, `manualSignatureInput`. **Required: everything except the photo** — Full Name, Position, Employee Number, Address, Phone Number, PhilHealth Number, SSS Number, TIN, HDMF, Contact Name, Contact Address, Contact Number, **and the Signature** (verified against `missingFields` in `commitManualEntry`, 2026-09-08). Photo zone visible; only the photo is optional. The signature zone shows a red `*` on its section header and reads "Click to upload signature" + "PNG preferred for transparent backgrounds", matching the photo zone; its label text lives in the `SIG_LABEL` constant (the edit modal keeps its own `_sigHint` "(optional)" because replacing an existing signature genuinely is).
